...
def video_call(request):
    return render(request,'hospital/video_call.html')
...
def get_remarks(request):
    remarks = models.Remarks.objects.all()
    return render(request, 'hospital/list_remark.html', {'remarks': remarks})
...
def remarks_view(request):
    if request.method == 'POST':
        form = forms.RemarksForm(request.POST)
        if form.is_valid():
            form.save()   
    else:
        form = forms.RemarksForm()
    return render(request, 'hospital/remarks.html', {'form': form})
...
def get_post(request):
    ...
    print(post)
    return render(request,"hospital/blog_get.html",{'post':post})
...
def create_post(request):
    ...
    return render(request,'hospital/blog_post.html',{'form':form})
